...
$pseudo=$_SESSION['pseudo'];
 ...
 $prod= $DB->query('SELECT id_produit, validcomande.designation as designation, stock.quantity as qtites, validcomande.quantite as qtite, pachat, pvente, previent, frais, prix_revient FROM validcomanderesto inner join stock on stock.id=id_produit order by(validcomande.id)');
        ...
        if ($_SESSION['motif']!="" AND $_POST['client']!="") {
            ...
            if (isset($_POST['prix_reel']) AND $_POST['prix_reel']!="") {
                ...
                if (($_POST['prix_reel'])<$_POST['montantc']) {?>
                    ...
                    $DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero, date_versement) VALUES(?, ?, ?, ?, now())', array($_POST['compte'], -$_POST['montantc'], "paiement (".'cmd'.$numero_commande.')', 'cmd'.$numero_commande));

                    $DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero, date_versement) VALUES(?, ?, ?, ?, now())', array(1, -$_POST['frais'], "paiement frais (".'cmd'.$numero_commande.')', 'cmd'.$numero_commande));
                    ...
                    $DB->delete('DELETE FROM validcomanderesto'); //pour supprimer les produits validés 
