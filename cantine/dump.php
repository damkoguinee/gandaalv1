<?php require 'header1.php';
    ...
    if ($_SESSION['date1']==$_SESSION['date2']) {
      ...
      $DB->insert('INSERT INTO banqueresto (id_banque, libelles, numero, montant, date_versement) VALUES (?, ?, ?, ?, ?)', array(1, 'cloture', 1, -$difference, $datev));
    ...
    $restep=$DB->querys("SELECT SUM(Total) AS totc, SUM(montantpaye) AS montc, SUM(reste) as reste, mode_payement, SUM(remise) AS remc, sum(fraisup) as frais FROM payementresto WHERE DATE_FORMAT(date_cmd, \"%Y%m%d\")= '{$dateformat}' AND etat= '{$etat}'");
