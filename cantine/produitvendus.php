            ...
            <div class="row text-center"><?php 
              ...
              foreach ($prodcat as$valuecat) {
                ...
                if (isset($_POST['magasin']) and $_POST['magasin']=='general') {
                  ...
                  $products =$DB->querys("SELECT commande.id as id FROM commande inner join stock on stock.id=id_produit inner join payementresto on payement.num_cmd=commande.num_cmd WHERE DATE_FORMAT(date_cmd, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") <= '{$_SESSION['date2']}' and idcat= '{$valuecat->id}' ");

                }else{

                  $products =$DB->querys("SELECT commande.id as id FROM commande inner join stock on stock.id=id_produit inner join payementresto on payement.num_cmd=commande.num_cmd WHERE vendeur='{$_SESSION['idpseudo']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") <= '{$_SESSION['date2']}' and idcat= '{$valuecat->id}' ");    
                }  ?>
