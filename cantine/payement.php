	...
	if ($panier->espace($_POST['reste'])<=0) {
		...
		if (!empty($_POST['clientvip'])) {
			...
			if (empty($datev)) {
				...
				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array($init.$numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment));
			}else{

				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($init.$numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment, $datev));
			}
