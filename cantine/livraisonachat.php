          ...
          <table class="table table-hover table-bordered table-striped table-responsive text-center">
            ...
            <tbody><?php 
              ...
              if (isset($_POST['j1'])) {
                ...
                $products= $DB->query("SELECT *FROM payementresto inner join client on client.id=num_client  WHERE etatliv!='{$etatliv}' and (DATE_FORMAT(date_cmd, \"%Y-%m-%d \")='{$_POST['j1']}') order by(date_cmd) desc LIMIT 10");

              }elseif (isset($_GET['searchnlclient'])) {

                $products= $DB->query("SELECT *FROM payementresto inner join client on client.id=num_client  WHERE etatliv!='{$etatliv}' and (num_client='{$_GET['searchnlclient']}') order by(date_cmd) desc LIMIT 10");

              }elseif (isset($_POST['numcmd'])) {

                $products= $DB->query("SELECT *FROM payementresto inner join client on client.id=num_client  WHERE etatliv!='{$etatliv}' and (num_cmd='{$_POST['numcmd']}') order by(date_cmd) desc LIMIT 10");

              }else{

                $products= $DB->query("SELECT *FROM payementresto inner join client on client.id=num_client  WHERE etatliv!='{$etatliv}' order by(date_cmd) desc LIMIT 30");
              }
