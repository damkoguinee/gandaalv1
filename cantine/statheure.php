...
$sql_annees = $DB->query('SELECT YEAR(date_cmd)as ANNEE FROM payementresto GROUP BY ANNEE');
...
foreach ($sql_annees as $row_annees) {
    ...
    $sql_ventes_par_mois = $DB->query('SELECT MONTH(date_cmd)as MOIS, COUNT( `id` ) AS NOMBRE_VENTE, SUM(Total/1000) AS PRODUIT_VENTE FROM payementresto where year(date_cmd)=:annee GROUP BY MOIS', array('annee'=>$row_annees->ANNEE));
