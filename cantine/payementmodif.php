...
if ($totalcomverif==$totachatverif) {//Vérification du totalcmd
	...
	if ($panier->espace($_POST['reste'])<=0) {
		...
		$DB->delete('DELETE from payementresto WHERE num_cmd=?', array($numero_commande));
		$DB->delete('DELETE from banqueresto WHERE numero=?', array('vente'.$numero_commande));
		$DB->delete('DELETE from banqueresto WHERE numero=?', array('fsup'.$numero_commande));
		...
		if (!empty($_POST['clientvip'])) {
			...
			if (empty($datev)) {
				...
				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array($numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment));
			}else{

				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment, $datev));
			}

			if (empty($datev)) {
				$DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero,  date_versement) VALUES(?, ?, ?, ?, now())', array($_POST['compte'], $montantpaye, 'vente n°'.$init.$numero_commande, 'vente'.$numero_commande));

			}else{

				$DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero,  date_versement) VALUES(?, ?, ?, ?, ?)', array($_POST['compte'], $montantpaye, 'vente n°'.$init.$numero_commande, 'vente'.$numero_commande, $datev));
		...
		if (empty($client)) {
			...
			if (empty($datev)) {
		    	...
		    	$DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero,  date_versement) VALUES(?, ?, ?, ?, ?)', array($_POST['compte'], $montantpaye, 'vente n°'.$numero_commande, 'vente'.$init.$numero_commande, $datev));
