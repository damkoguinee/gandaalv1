...
if (isset($_GET['delpc'])) {
	...
	$DB->delete('DELETE FROM validcomanderesto WHERE id = ?', array($_GET['delpc']));
}

if (isset($_GET['desig'])) {

	$prodvalidcverif = $DB->querys("SELECT quantite FROM validcomanderesto where id_produit='{$_GET['idc']}' and pseudo='{$_SESSION['idpseudo']}' ");

	if (empty($prodvalidcverif)) {
				
		$DB->insert('INSERT INTO validcomanderesto (id_produit, designation, quantite, pachat, pvente, previent, frais, etat, pseudo, datecmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array($_GET['idc'], $_GET['desig'], 1, $_GET['pa'], $_GET['pv'], 0, 0, 'paye', $_SESSION['idpseudo']));

	}else{

		$qtitesup=$prodvalidcverif['quantite']+1;

		$DB->insert('UPDATE validcomanderesto SET quantite=? where id_produit=? and pseudo=?', array($qtitesup, $_GET['idc'], $_SESSION['idpseudo']));
...
if (isset($_POST['modifcom']) or isset($_GET['modifcom'])) {
	...
	$DB->insert('UPDATE validcomanderesto SET quantite=?, pachat=?, frais=?, pvente=? where id=? and pseudo=?', array($_POST['quantity'], $_POST['pachat'], $_POST['frais'], $_POST['pvente'], $_POST['id'], $_SESSION['idpseudo']));
}

$prodvalidc = $DB->query('SELECT validcomande.id as id, id_produit, validcomande.quantite as quantite, validcomande.designation as designation, pvente, pachat, prix_achat, prix_vente, frais FROM validcomanderesto inner join stock on stock.id=validcomande.id_produit order by(validcomande.id)desc');
