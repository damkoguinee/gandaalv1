      ...
      <div class="reglement">
        ...
        <form method="post"  action="emprunt.php" id="naissance" style="width: 92%; margin: 2px;">
          ...
          if (isset($_POST['num_cmd'])){
            ...
            if ($_POST['montant']!=0) {
              ...
              $product = $DB->query('SELECT num_cmd FROM payementresto WHERE num_cmd=:num_cmd', array('num_cmd' => $_POST['num_cmd']));

              if(empty($product)){
                echo "Ce Numero est incorrect";        
              }else{

                $productpay = $DB->query('SELECT * FROM payementresto  WHERE num_cmd =:Num', array('Num'=>$_POST['num_cmd']));                    
                ...
                if ($product->etat=="credit") {
                  ...
                  if ($_POST['montant']>$reste) {
                    ...
                    $DB->insert('UPDATE payementresto SET montantpaye= ? , reste=? , mode_payement= ?, date_regul=? WHERE num_cmd = ?', array($montantpaye, $reste, $_POST['mode_payement'], $date, $_POST['num_cmd']));
                    ...
                    $productpaye=$DB->query('SELECT * FROM payementresto  WHERE num_cmd =:Num', array('Num'=>$_POST['num_cmd']));

                    foreach ($productpaye as $product):

                      if (($product->Total-$product->remise)== $product->montantpaye) {

                        $DB->insert('UPDATE payementesto SET etat= ? , date_regul=? WHERE num_cmd = ?',array('totalite', $date, $_POST['num_cmd']));
          ...
          $productable = $DB->query('SELECT num_cmd, client, Total, remise, reste, montantpaye, DATE_FORMAT(date_cmd, \'%d/%m/%Y \')AS DateTemps FROM payementresto  WHERE etat=:Etat ORDER BY (date_cmd)DESC', array('Etat'=>$Etat));
        ...
        if (isset($_POST['id'])){
          ...
          if ($_POST['montant_dec']!=0) {
            ...
            $product = $DB->query('SELECT id FROM decaissementresto WHERE id=:num_cmd', array('num_cmd' => $_POST['id']));
            ...
            if(empty($product)){
              ...
              $productdec = $DB->query('SELECT * FROM decaissementresto  WHERE id =:ID', array('ID'=>$_POST['id']));
              ...
              if ($product->etat=="credit") {
                ...
                if($_POST['montant_dec']>$reste) {
                  ...
                  $date = date('y-m-d H:i');              
                  $DB->insert('UPDATE decaissementresto SET montant=?, payement= ?, date_regul=? WHERE id = ?', array($montant, $_POST['mode_payement'], $date, $_POST['id']));
                  ...
                  $DB->insert('INSERT INTO historique (num_cmd, montant, payement, nom_client, date_cmd, remboursement, date_regul) VALUES(?, ?, ?, ?, ?, ?, now())', array($_POST['id'], $_POST['montant_dec'], $_POST['mode_payement'], $product->client, $product->date_payement, $remb));
                    ...
                    foreach ($productdec as $product):
                      ...
                      if (($product->montant)== $product->prix_reel) {
                        ...
                        $DB->insert('UPDATE decaissementresto SET etat= ? , date_regul=? WHERE id= ?', array('clos', $date, $_POST['id']));
          ...
          $productlis = $DB->query('SELECT id, prix_reel,montant, monnaie, motif, client, DATE_FORMAT(date_payement, \'%d/%m/%Y \')AS DateTemps FROM decaissementresto  WHERE etat=:Etat ORDER BY (date_payement)DESC', array('Etat'=>$Etat));
          if (empty($productlis)) {
