...
<page backtop="10mm" backleft="10mm" backright="10mm" backbottom="10mm" footer="page;">
  ...
  if (isset($_GET['numdec'])) {
    ...
    if (isset($_GET['numdec'])) {
      ...
      $array = $DB->querys("SELECT max(id) FROM decaissementresto "); 
      $Numv=$array['max(id)'];

    }

    $payement = $DB->querys('SELECT numdec, montant, payement, coment, DATE_FORMAT(date_payement, \'%d/%m/%Y \à %H:%i:%s\')AS DateV FROM decaissementresto WHERE decaissement.id= ?', array($Numv));?>
