...
$maximum = $DB->querys('SELECT max(id) AS max_id FROM payementresto ');


$numero_commande = $date + $maximum['max_id'] + 1;

$numerot= $DB->querys("SELECT (COUNT(id)+1) AS nbre_cmd  FROM payementresto where DATE_FORMAT(date_cmd, \"%Y%m%d\")='{$_SESSION['date1']}'");
...
$coment=$_POST['coment'];
$prodvente = $DB->querys("SELECT *FROM tablevalide where idtable='{$_SESSION['tableresto']}' and pseudop='{$pseudo}' ");
...
$total=$panier->totalcomtable()+$fraisup;

$totalcomverif=number_format($panier->totalcomtable(),0,',','');
...
if ($totalcomverif==$totachatverif) {//Vérification du totalcmd
	...
	if ($panier->espace($_POST['reste'])<=0) {
		...
		$idtable=$panier->nomTable($prodvente['idtable'])[0];
		$reste=0;
		...
		if (!empty($_POST['clientvip'])) {
			...
			if (empty($datev)) {
				...
				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, idtable, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array($init.$numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment, $idtable));
			}else{

				$DB->insert('INSERT INTO payementresto (num_cmd, num_ticket, Total, fraisup, montantpaye, remise, reste, etat, mode_payement, vendeur, position, etatliv, num_client, coment, idtable, date_cmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($init.$numero_commande, $numticket, $total, $fraisup, $montantpaye, $remise, $reste, $etat, $_POST['mode_payement'], $pseudo, $position, $etatliv, $numclient, $coment, $idtable, $datev));
			}

			if (empty($datev)) {
				$DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero,  date_versement) VALUES(?, ?, ?, ?, now())', array($_POST['compte'], $montantpaye, 'vente n°'.$init.$numero_commande, 'vente'.$init.$numero_commande));

			}else{

				$DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero,  date_versement) VALUES(?, ?, ?, ?, ?)', array($_POST['compte'], $montantpaye, 'vente n°'.$init.$numero_commande, 'vente'.$init.$numero_commande, $datev));
	...
	$products= $DB->query("SELECT id_produit as id, stock.nom as designation, stock.quantity as qtites, tablecommande.quantite as qtite, pvente, prix_achat, prix_revient, prix_vente, stock.type as type, idingredient, qtiteingredient FROM tablecommande inner join stock on stock.id=id_produit where idtable='{$_SESSION['tableresto']}' and pseudov='{$_SESSION['idpseudo']}' order by(tablecommande.id)");
    ...
    $DB->delete("DELETE FROM tablecommande where idtable='{$_SESSION['tableresto']}' and pseudov='{$pseudo}' ");
	$DB->delete("DELETE FROM tablevalide where idtable='{$_SESSION['tableresto']}' and pseudop='{$pseudo}' ");
