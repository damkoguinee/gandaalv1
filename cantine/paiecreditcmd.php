...
if (isset($_POST['paieunit']) or isset($_GET['paiepart'])){
  ...
  if (isset($_POST['paieunit']) and isset($_POST['montant']) ) {
    ...
    if(empty($product)){?>
      ...
      if ($reste!="0") {
        ...
        if ($_POST['montant']>$reste) {?>
          ...
          $DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero, date_versement) VALUES(?, ?, ?, ?, now())', array($_POST['compte'], -$_POST['montant'], 'paiement '.$product['numcmd'], $product['numcmd']));
    ...
    foreach ($prodchoix as $value) {
      ...
      $DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero, date_versement) VALUES(?, ?, ?, ?, now())', array($_GET['compte'], -$montremb, 'paiement '.$value->numero, $value->numero));
    ...
    foreach ($prodpaie as $value) {
      ...
      $DB->insert('INSERT INTO banqueresto (id_banque, montant, libelles, numero, date_versement) VALUES(?, ?, ?, ?, now())', array($_POST['compte'], -$resteapayer, 'paiement '.$value->numcmd, $value->numcmd));
