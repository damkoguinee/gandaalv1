    ...
    <table class="table table-hover table-bordered table-striped table-responsive">
      ...
      foreach ($panier->modep as $key=> $produc){
        ...
        if (isset($_POST['magasin']) and $_POST['magasin']=='general') {
          ...
          $product =$DB->querys("SELECT SUM(Total) AS TOT, SUM(montantpaye) as montantp, SUM(remise) AS REM, SUM(fraisup) AS frais, mode_payement FROM payementrest WHERE vendeur='{$_SESSION['idpseudo']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") <= '{$_SESSION['date2']}'  AND mode_payement= '{$produc}'");    
        }
      ...
      if (isset($_POST['magasin']) and $_POST['magasin']=='general') {
        ...
        $restep=$DB->querys("SELECT SUM(Total) AS totc, SUM(montantpaye) AS montc, SUM(reste) as reste, mode_payement, SUM(remise) AS remc, sum(fraisup) as frais FROM payementresto WHERE vendeur='{$_SESSION['idpseudo']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_cmd, \"%Y%m%d\") <= '{$_SESSION['date2']}'  AND etat= '{$etat}'");    
      }
