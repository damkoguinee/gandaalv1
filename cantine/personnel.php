    	...
    	<div class="container-fluid mt-3"><?php 
			...
			if(isset($_POST['ajouteen'])){
				...
				if($_POST['nom']!=""  and $_POST['perso']!=""){
					...
					$nb=$DB->querys('SELECT id from personnelresto where nom=:nom', array(
						'nom'=>$nom
						));
						...
						if(!empty($nb)){?>
							...
							$nb=$DB->querys('SELECT max(id) as id from personnelresto');


							$matricule=$nb['id']+1;

							$DB->insert('INSERT INTO personnelresto(identifiant, nom, telephone, agence, pseudo, mdp, level, statut, dateenreg) values(?, ?, ?, ?, ?, ?, ?, ?, now())', array($matricule, $nom, $phone, 'BoutiqueB3', $pseudo, $mdp, $niveau, $type));
					...
					<div class="col-sm-12 col-md-12">
					    ...
					    <form id="naissance" method="POST" action="personnel.php" style="width: 60%;">
					    	...
					    	<fieldset><legend>Modifier un personnel</legend><?php
								...
								$prodm=$DB->querys('SELECT * from personnelresto  where id=:mat', array('mat'=>$_GET['modif_en']));?>
				...
				if(isset($_POST['modifen'])){
					...
					$DB->insert('UPDATE personnelresto SET nom = ?, telephone=?, pseudo=?, mdp=?, level=?, statut=?  WHERE id = ?', array($nom, $phone, $pseudo, $mdp, $niveau, $type, $_POST['id']));?>	
			    ...
			    if (isset($_GET['enseig']) or isset($_POST['ajouteen'])  or isset($_GET['del_en']) or isset($_GET['del_pers']) or isset($_POST['modifen']) or isset($_GET['matiereen']) or isset($_GET['personnel']) or isset($_GET['payempcherc'])) {
			    	...
			    	if (isset($_GET['del_pers'])) {
			          ...
			          $DB->delete('DELETE FROM personnelresto WHERE id = ?', array($_GET['del_pers']));?>
	        		...
	        		$statut='admin';
					$prodm=$DB->query("SELECT * from personnelresto where statut!='{$statut}'");?>
